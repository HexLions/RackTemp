...
import { api, ApiError, HttpsSettings, NetworkInfo } from "../../api/client";
...
export default function NetworkSection() {
  ...
  function load() {
    ...
    api.get<NetworkInfo>("/system/network-info").then(setNetwork);
  }
    ...
    <>
      <div className="card">
        ...
        <p className="hint" style={{ marginTop: -4, marginBottom: 12 }}>
          Use one of these to reach the dashboard from another device on the LAN, and to fill in the
          IP field on a sensor's setup portal — not "localhost", which only ever means the machine
          you're browsing from itself (the Windows tray app in particular always shows "localhost"
          in its own window, regardless of which machine the server actually runs on).
        </p>
        {network && network.ips.length > 0 ? (
          network.ips.map((ip) => <CopyField key={ip} label="Address" value={`https://${ip}:${network.port}`} />)
        ) : (
          <p className="hint">{network ? "No LAN address detected on this machine." : "Loading…"}</p>
        )}
      </div>
