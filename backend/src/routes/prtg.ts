...
//
// LimitMinError is 0.5, not 1: confirmed against a real PRTG instance that
// a limit exactly equal to the "good" value (1) makes PRTG treat the
// channel as errored even while online, i.e. LimitMinError's comparison is
// inclusive (<=), not strictly-below. 0.5 sits strictly between the two
// possible values (0 offline / 1 online) so it's unambiguous either way.
